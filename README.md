...
*   **Controller (`controllers/SearchController.py`)**: The core orchestration layer. It manages the flow of:
    1.  Tokenizing the user query via `text_processor`.
    2.  Fetching relevant postings and metadata via `data_provider`.
    3.  Calculating relevance scores via `ranker`.
...
**Response Format:**
Returns a JSON list of tuples, where each tuple contains `(doc_id, title)`.
```json
[
