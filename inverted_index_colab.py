...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            # build full path
            f_path = str(self._base_dir / f_name)

            if f_path not in self._open_files:
                self._open_files[f_path] = open(f_path, 'rb')

            f = self._open_files[f_path]
            f.seek(offset)
...
def read_posting_list(inverted, base_dir, w):
  """
  Read posting list for term w from disk.

  Parameters
  ----------
  inverted : InvertedIndex
      Loaded inverted index metadata (df, posting_locs).
  base_dir : str or Path
      Directory containing the .bin posting files.
  w : str
      Term to retrieve posting list for.
